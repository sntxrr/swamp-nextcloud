...
| --- | --- | --- |
| [`@sntxrr/nextcloud`](extensions/models/README.md) | model | Health, version drift, setup checks and app updates, plus `occ` maintenance (maintenance mode, database repair, app updates) that is a dry run unless `apply=true` |
...
```bash
~/.swamp/deno/deno test extensions/models/nextcloud_instance_test.ts
